...
# Identify large price movements
data['Price_Change'] = data['Close'].diff()

# Find where significant price changes and dominant frequencies align
price_movement_threshold = data['Price_Change'].std()  # Example threshold
large_movements = data[(abs(data['Price_Change']) > price_movement_threshold)]

# Compare these large movements with the filtered volume oscillator to see if dominant frequencies coincide
plt.figure(figsize=(14, 7))
plt.plot(data.index, data['Filtered_Vol_Osc'], label='Filtered Volume Oscillator')
plt.scatter(large_movements.index, large_movements['Filtered_Vol_Osc'], color='red', label='Significant Price Movement')
plt.title('Filtered Volume Oscillator with Significant Price Movements')
plt.legend()
plt.show()
